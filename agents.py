...
def call_gpt(system_prompt, user_input):
    ...
    try:
        response = client.chat.completions.create(
            model="gpt-4o",  # Switch to "gpt-3.5-turbo" if you don't have GPT-4 access
            messages=[
        ...
        # --- THE FIX: Clean Markdown Formatting ---
...
class SentinelAgent:
    def analyze(self, event):
        ...
        try:
            ...
            print(f"{Fore.RED}Error parsing Sentinel JSON. Raw output: {response}{Style.RESET_ALL}")
...
class HistorianAgent:
    def check_context(self, event, emr_data, sentinel_alert):
        ...
        Check the EMR below. Does the patient's history, meds, or profile EXPLAIN this anomaly?
        If yes, OBJECT to the alert. If no, CONFIRM it.
        
        Output JSON ONLY:
        {{
            "verdict": "CONFIRM" or "OBJECT",
            "explanation": "Why you think this is safe or unsafe based on the EMR."
        }}
        """
        ...
        try:
            ...
            print(f"{Fore.RED}Error parsing Historian JSON. Raw output: {response}{Style.RESET_ALL}")
            return {"verdict": "CONFIRM", "explanation": "Error parsing AI response"}

# --- AGENT 3: THE TRIAGE OFFICER (Decision) ---
class TriageAgent:
    def decide(self, sentinel_data, historian_data):
        print(f"{Fore.MAGENTA}👮 TRIAGE OFFICER is judging...{Style.RESET_ALL}")
        
        prompt = """
        You are the TRIAGE OFFICER. You are the boss.
        Your goal is to prevent Alarm Fatigue.
        
        1. Sentinel says: {sentinel_reason} (Severity: {sentinel_sev})
        2. Historian says: {historian_verdict} ({historian_exp})
        
        DECISION RULES:
        - If Historian OBJECTS and provides a valid medical reason, suppress the alarm (YELLOW LOG).
        - If Historian CONFIRMS or the severity is CRITICAL (life threatening), sound the alarm (CODE RED).
        ...
        try:
            ...
            print(f"{Fore.RED}Error parsing Triage JSON. Raw output: {response}{Style.RESET_ALL}")
